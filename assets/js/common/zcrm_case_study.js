...
function zcsNxt_slide(d) {
    ...
    if (e == "nxt") {
        ...
        if (e == "prv") {
            ...
            if (e == "auto") {
                $(".cx-testiLst:first-child").addClass("active").siblings().removeClass("active")
            }
        }
    }
...
function zcs_pagination(k) {
    ...
    if (typeof k != "undefined") {
        ...
        if (g > fixed_length) {
            for (var d = 1; d <= f; d++) {
                a += '<li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + d + '">' + d + "</li>";
                if (d == f) {
                    a += '<li class="pgi_numbers mid_dots">' + h + '</li><li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + e + '">' + e + "</li>"
                }
            }
            b.append(a);
...
function pageRendering(m, n, r) {
    ...
    if (c) {
        if (v > fixed_length) {
            ...
            if (f > 0 && e <= h) {
                for (var s = f; s <= e; s++) {
                    d += '<li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + s + '">' + s + "</li>"
                }
                if (g < 551) {
                    if (h < 4) {
                        d = d
                        ...
                        if (h >= 4) {
                            if (c == p || c == p + 1) {
                                l = '<li class="pgi_numbers mid_dots">' + u + '</li><li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + h + '">' + h + "</li>";
                                d = d + l
                            } else {
                                if (c == h || c == h - 1) {
                                    l = '<li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + p + '">' + p + '</li><li class="pgi_numbers mid_dots">' + u + "</li>";
                                    d = l + d
                                } else {
                                    if (c > p && c < h) {
                                        b = '<li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + p + '">' + p + '</li><li class="pgi_numbers mid_dots">' + u + "</li>";
                                        l = '<li class="pgi_numbers mid_dots">' + u + '</li><li class="pgi_numbers" onclick="pageRendering(drained_data, this, event)" data-num="' + h + '">' + h + "</li>";
                                        d = b + d + l
                                    }
                                }
...
function zcs_dataRendering(e) {
    ...
    if (urlEleActive) {
        history.pushState(null, null, pageUrlEle + UrlEle + a + "tab_fltr=" + b + "&viewEle=" + viewFltr + "&" + countryIndex + "&" + IndustryIndex + "&" + usersIndex)
    }
    urlEleActive = true;
    ...
    $.each(tidy_Data.customers, function(z) {
        ...
        var t = '<div class="vid_spkrGrp"><span class="vid_spkrname">' + tidy_Data.customers[v].cs_name + '</span><span class="vid_spkrdesig">' + tidy_Data.customers[v].cs_company + "</span></div>";
        if (tidy_Data.customers[v].cs_img.length == 0) {
            var o = "zcs_spkrNo"
        } else {
            var o = "zcs_spkrYes"
        }
        if (tidy_Data.customers[v].cs_label == "Review" || tidy_Data.customers[v].cs_label == "Social") {
            var h = "review_spkr"
        } else {
            h = ""
        }
        if (tidy_Data.customers[v].cs_name.length == 0 && tidy_Data.customers[v].cs_company.length == 0 || tidy_Data.customers[v].cs_label == "Videos") {
            var q = ""
        } else {
            q = '<div class="cs_skrGrp ' + h + '"><span class="skrGrp_lft ' + o + '">' + tidy_Data.customers[v].cs_img + '</span><span class="skrGrp_rht"><span class="skrGrp_name">' + tidy_Data.customers[v].cs_name + '</span><span class="skrGrp_desig">' + tidy_Data.customers[v].cs_company + "</span></span></div>"
        }
        if (tidy_Data.customers[v].cs_label == "Videos") {
            ...
            var r = '<div class="cs_videoSec vimvideo" data-video="' + tidy_Data.customers[v].vid_url + '">' + t + '<img class="' + g + ' vid_thumb" data-src="' + tidy_Data.customers[v].videothumb + '" alt="' + tidy_Data.customers[v].cs_name + " | Zoho " + u + ' Customer" width="350px" height="220px"></div>'
        } else {
            r = ""
        }
        if (tidy_Data.customers[v].cs_label == "review" || tidy_Data.customers[v].cs_label == "social") {
            var j = '<div><a class="zcs_sites" href="' + tidy_Data.customers[v].read_more + '" target="_blank" rel="noopener">' + tidy_Data.customers[v].link_txt + "</a></div>"
        } else {
            j = '<div><a class="zcs_lnk" href="' + tidy_Data.customers[v].read_more + '" target="_blank" rel="noopener">' + tidy_Data.customers[v].link_txt + '<span class="arrow"></span></a></div>'
        }
        var m = "",
            s, w = tidy_Data.customers[v].cs_msg.length,
            p = "";
        if (w > 320) {
            if (w - 320 > 100) {
                s = '<div class="zct_Desc_grp showmore">' + tidy_Data.customers[v].cs_msg + '</div><span class="zct_showmore" onclick="zct_showmore(this)">Show more...</span>'
            } else {
                s = '<div class="zct_Desc_grp">' + tidy_Data.customers[v].cs_msg + "</div>"
            }
        } else {
            s = '<div class="zct_Desc_grp">' + tidy_Data.customers[v].cs_msg + "</div>"
        }
        if (tidy_Data.customers[v].cs_logo != "") {
            ...
            if (tidy_Data.customers[v].cs_logoTitle != "") {
                m = '<span class="cust_logo_txt">' + tidy_Data.customers[v].cs_logoTitle + "</span>"
            }
        }
        if (customvar.productName != "desk") {
            p = tidy_Data.customers[v].country[0]
        }
        cs_oneData = '<li class="cs_lstBx flr_show ' + p + " " + tidy_Data.customers[v].cs_label + '"><div class="cs_lstInner"><div class="cs_testiSec">' + m + s + j + q + "</div> " + r + "</div></li>";
        var x, n, l, k = [];
        if (dev_W > 1024) {
            x = 3
        } else {
    ...
    if (window.location.origin == "https://www.bigin.com") {
        $(".skrGrp_img,.zct_logo").each(function() {
            $(this).attr("data-bg", "https://www.zohowebstatic.com" + $(this).attr("data-bg"))
        })
    }
    ...
    $(".cs_videoSec").each(function() {
        var g = $(this);
        if ((g.attr("data-video").length == 0) || (g.find("img").attr("data-src").length == 0)) {
            g.css("display", "none")
        } else {
...
function Flr_rendering() {
    ...
    for (var j = 0; j < h; j++) {
        ...
        cs_indus += '<span class="cs_industry"><label class="cs_lable"><input type="checkbox" name="industry" data-industry="' + d + '">' + e[j] + '<span class="cs_sprt cs_check"></span></label></span>'
    }
    for (var j = h; j < e.length; j++) {
        var d = e[j].replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
        cs_indus1 += '<span class="cs_industry"><label class="cs_lable"><input type="checkbox" name="industry" data-industry="' + d + '">' + e[j] + '<span class="cs_sprt cs_check"></span></label></span>'
    }
    cs_user = '<span class="cs_users"><label class="cs_lable"><input type="checkbox" name="users" data-users="50_and_below">50 and below<span class="cs_sprt cs_check"></span></label></span><span class="cs_users"><label class="cs_lable"><input type="checkbox" name="users" data-users="51-200">51-200<span class="cs_sprt cs_check"></span></label></span><span class="cs_users"><label class="cs_lable"><input type="checkbox" name="users" data-users="201-500">201-500<span class="cs_sprt cs_check"></span></label></span><span class="cs_users"><label class="cs_lable"><input type="checkbox" name="users" data-users="Above_500">Above 500<span class="cs_sprt cs_check"></span></label></span>';
    for (var f = 0; f < b.length; f++) {
        var d = b[f].replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
        cs_cntry += '<span class="cs_country"><label class="cs_lable"><input type="checkbox" name="country" data-country="' + d + '">' + b[f] + '<span class="cs_sprt cs_check"></span></label></span>'
    }
    if (customvar.productName != "desk") {
        for (var g = 0; g < c.length; g++) {
            var d = c[g].replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
            cs_business += '<span class="cs_business"><label class="cs_lable"><input type="checkbox" name="business" data-business="' + d + '">' + c[g] + '<span class="cs_sprt cs_check"></span></label></span>'
        }
...
function flr_smart() {
    ...
    for (var z = 0; z < r; z++) {
        for (var y = 0; y < j; y++) {
            var J = tidy_Data.customers[y].industry.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
            if (N[z] == J) {
                c.push(y)
            }
        }
    ...
    for (var z = 0; z < w; z++) {
        for (var y = 0; y < j; y++) {
            var b = tidy_Data.customers[y].region.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
            if (I[z] == b) {
                M.push(y)
            }
        }
    ...
    for (var z = 0; z < n; z++) {
        for (var y = 0; y < j; y++) {
            var A = tidy_Data.customers[y].business_process.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
            if (s[z] == A) {
                H.push(y)
            }
        }
    ...
    for (var z = 0; z < v; z++) {
        for (var y = 0; y < j; y++) {
            var d = tidy_Data.customers[y].user_count.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
            if (o[z] == d) {
                L.push(y)
            }
        }
...
function error_prevent(l, m) {
    ...
    for (var e = 0; e < l.length; e++) {
        var g = l[e];
        var k = tidy_Data.customers[g].region.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_"),
            d = tidy_Data.customers[g].user_count.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
        if (tidy_Data.customers[g].business_process != "" && tidy_Data.customers[g].business_process != undefined) {
            ele_business = tidy_Data.customers[g].business_process.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_")
        } else {
            ele_business = ""
        }
        ele_Ind = tidy_Data.customers[g].industry.replace(/[&\/\\#, +()$~%.'":*?<>{}]/g, "_");
        n.push(ele_Ind);
        o.push(k);
        c.push(d);
        f.push(ele_business)
    ...
    for (var e = 0; e < j.length; e++) {
        $(".cs_lable input[data-country =" + j[e] + "]").parent().addClass("zct_enable")
    }
    for (var e = 0; e < a.length; e++) {
        $(".cs_lable input[data-users =" + a[e] + "]").parent().addClass("zct_enable")
    }
    for (var e = 0; e < h.length; e++) {
        $(".cs_lable input[data-industry =" + h[e] + "]").parent().addClass("zct_enable")
    }
    for (var e = 0; e < b.length; e++) {
        $(".cs_lable input[data-business =" + b[e] + "]").parent().addClass("zct_enable")
    }
    if ((m[0] >= 1 && m[1] >= 1) || (m[1] >= 1 && m[2] >= 1) || (m[2] >= 1 && m[3] >= 1) || (m[3] >= 1 && m[0] >= 1) || (m[3] >= 1 && m[2] >= 1 && m[1] >= 1 && m[0] >= 1)) {
        $(".cs_country, .cs_users, .cs_industry, .cs_business").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
    } else {
        if (m[2] < 1 && m[0] < 1) {
            $(".cs_business, .cs_industry").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
        } else {
            if (m[1] < 1 && m[0] < 1) {
                $(".cs_country, .cs_industry").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
            } else {
                if (m[2] < 1 && m[1] < 1) {
                    $(".cs_country, .cs_business").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
                } else {
                    if (m[3] < 1 && m[1] < 1) {
                        $(".cs_country, .cs_users").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
                    } else {
                        if (m[3] < 1 && m[2] < 1) {
                            $(".cs_business, .cs_users").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
                        } else {
                            if (m[0] < 1 && m[3] < 1) {
                                $(".cs_industry, .cs_users").find(".cs_lable:not(.zct_enable)").addClass("zct_disable")
                            }
                        }
                    }
                }
...
function Flr_TagShow() {
    var b = "";
    var f, g, a = $(".cs_lable input:checked");
    g = a.length;
    if (g > 0) {
        for (var e = 0; e < g; e++) {
            f = a.eq(e).attr("name");
            var d, c = a.eq(e).parent().text();
            d = '<span class="flr_tags" data-tag=' + f + ">" + c + '<span class="flr_tagclose"></span></span>';
            b += d
        }
        $(".flr_tagtxt").html(b)
    }
...
$(document).on("click", ".flr_tagclose", function() {
    var c, a, b = $(this).parent(".flr_tags"),
        d = $(".flr_tags").length;
    if (d <= 1) {
...
function urlBasedActive() {
    ...
    if (urlValuesGet.has("tab_fltr") && urlValuesGet.has("page") && view_fltr == "less") {
