...
$(document).ready(function () {
  ...
  if ($(".pricing-main-con").hasClass("crmflex-con")) {
    if (windowWidth <= 1140 && windowWidth > 767) {
      $(".cxbox-main-con").each(function () {
        $(this)
          .find(".cxbox-ans-con .cxplan-initVal")
          .html($(this).find(".cxhdr-main-ele .cxplan-initVal").html());
  ...
  $(".testi-sldr-con").slick({
    ...
    dots: true,
    autoplaySpeed: 8500,
    responsive: [{ breakpoint: 992, settings: { adaptiveHeight: true } }],
  });
  if (windowWidth <= 992) {
  ...
  if (windowWidth <= 767) {
    $(".cmp_ftr_lists").each(function (a) {
      ...
      $(".prce-features-list").each(function () {
        ...
        b.each(function (d) {
          c.find(".prce-ftrs-inrlst li:eq(" + d + ")").append(
            $(this).find(".pricing-tooltip-sec").clone()
          );
        });
      });
    });
    ...
    $(
      '<img class="zcp_lazy business_icons" data-src="/sites/zweb/images/crm/zpricing-brands-mob.svg" alt="customer-logos">'
    ).insertAfter(".business_sec .scroll_top_prcng.access-btn");
    zcp_lazy();
    if ($(".table-right-main").length > 0) {
      ...
      setTimeout(function () {
        $(".table-right .slick-next,.tableheading-fixed .slick-next").append(
          '<img src="/sites/zweb/images/crm/images/support-slick-arrow.svg" alt="Next Arrow">'
        );
        $(".table-right .slick-prev,.tableheading-fixed .slick-prev").append(
          '<img src="/sites/zweb/images/crm/images/support-slick-arrow.svg" alt="prev Arrow">'
        );
      }, 500);
    } else {
...
$(window).on("load", function () {
  setTimeout(function () {
    if ($("#zplan-crm-lyte1").attr("data-price") == "") {
      $("#zplan-lyte-enmonthly1,#zplan-lyte-ulmonthly1,#zplan-crm-lyte1").html(
        $("#zplan-crm-lyte").html()
      );
      $("#zplan-lyte-enmonthly1,#zplan-lyte-ulmonthly1,#zplan-crm-lyte1").attr(
        "data-price",
        $("#zplan-crm-lyte").attr("data-price")
      );
      if (windowWidth <= 767) {
        $(".cmp_Table").remove();
      }
    ...
    $(
      "#zplan-Standard2,#zplan-Standard3,#zplan-Standard4,#zplan-Standard6"
    ).html($("#zplan-Standard").html());
    $("#zplan-professional2,#zplan-professional3,#zplan-professional4").html(
      $("#zplan-professional").html()
    );
    $("#zplan-enterprise2,#zplan-enterprise3,#zplan-enterprise4").html(
      $("#zplan-enterprise").html()
    );
    $("#zplan-Ultimate2,#zplan-Ultimate3,#zplan-Ultimate4").html(
      $("#zplan-Ultimate").html()
    );
    $("#zplan-Standard5 .zcurrency-symbol").html(
      $("#zplan-Standard .zcurrency-symbol").html()
    );
    $("#zplan-Standard5 .z-price-text").html(
      $("#zplan-Standard .z-price-text").html()
    );
    ...
    $(
      "#zplan-Standard2,#zplan-Standard3,#zplan-Standard4,#zplan-Standard5,#zplan-Standard6"
    ).attr("data-price", $("#zplan-Standard").attr("data-price"));
    $("#zplan-professional2,#zplan-professional3,#zplan-professional4").attr(
      "data-price",
      $("#zplan-professional").attr("data-price")
    );
    $("#zplan-enterprise2,#zplan-enterprise3,#zplan-enterprise4").attr(
      "data-price",
      $("#zplan-enterprise").attr("data-price")
    );
    $("#zplan-Ultimate2,#zplan-Ultimate3,#zplan-Ultimate4").attr(
      "data-price",
      $("#zplan-Ultimate").attr("data-price")
    );
    $("#surveyplan-plus1").attr(
      "data-price",
      $("#surveyplan-plus").attr("data-price")
    );
    $("#projectsplan-premium1").attr(
      "data-price",
      $("#projectsplan-premium").attr("data-price")
    );
    $("#salesiqplan-basic1").attr(
      "data-price",
      $("#salesiqplan-basic").attr("data-price")
    );
    $("#socialplan-standard1").attr(
      "data-price",
      $("#socialplan-standard").attr("data-price")
    );
    $("#analyticsplan-basic1").attr(
      "data-price",
      $("#analyticsplan-basic").attr("data-price")
    );
    $("#zplan-email1-500").attr(
      "data-price",
      $("#zplan-email-500").attr("data-price")
    );
    $("#deskplan-standard1").attr(
      "data-price",
      $("#deskplan-standard").attr("data-price")
    );
    $(".cmpr-table-sec .zcurrency-symbol").html(
      $(".product-block .zcurrency-symbol").html()
    );
    $(".cx-price-popup .zcurrency-symbol").html(
      $("#zplan-Standard .zcurrency-symbol").html()
    );
    ...
    $("#zplan-Standard1").attr(
      "data-price",
      $("#zplan-Standard").attr("data-price")
    );
    $("#zplan-professional1").attr(
      "data-price",
      $("#zplan-professional").attr("data-price")
    );
    $("#zplan-enterprise1").attr(
      "data-price",
      $("#zplan-enterprise").attr("data-price")
    );
    $("#zplan-Ultimate1").attr(
      "data-price",
      $("#zplan-Ultimate").attr("data-price")
    );
    if ($(".pricing-main-con").hasClass("crmflex-con")) {
      ...
      $("#crmplan-Standard").attr(
        "data-price",
        $("#zplan-Standard").attr("data-price")
      );
      $("#crmplan-professional").attr(
        "data-price",
        $("#zplan-professional").attr("data-price")
      );
      $("#crmplan-enterprise").attr(
        "data-price",
        $("#zplan-enterprise").attr("data-price")
      );
      $("#crmplan-Ultimate").attr(
        "data-price",
        $("#zplan-Ultimate").attr("data-price")
      );
    }
...
$(window).scroll(function () {
  ...
  if (windowWidth <= 767) {
    ...
    if ($(".compar-main-table").length > 0) {
      if (
        $(window).scrollTop() > $(".compar-main-table").offset().top &&
        $(window).scrollTop() < $(".other-prsec").offset().top - 200
      ) {
...
function cmp_tbl_exp(b) {
  ...
  if (!c.find(".cmp_tbl_lsts").is(":visible")) {
    ...
    if (!$(".cmp_Table").hasClass("cmp_Fixed") && !c.hasClass("fst-elm")) {
      $("body,html").animate({ scrollTop: c.offset().top - a }, "500");
    }
...
$(document).on(
  "click",
  ".scroll_top_prcng,.z-bottom-cta-menu.withCW a",
  function (b) {
    b.preventDefault();
    var a = $(".pricing-wrap").offset().top;
    $("html,body")
      .stop()
      .animate({ scrollTop: a - 30 }, 700);
  }
);
$(".tab-lst").click(function () {
  ...
  $(".tab-con").fadeOut(0);
  $("#" + a).fadeIn();
  $("body,html").animate({ scrollTop: 40 }, "500");
});
...
$(document).on("click", ".zinner-heading.zhd", function () {
  ...
  if ($(this).parent("ul").hasClass("hide-elm")) {
    ...
    $(this)
      ...
      .find(".zinner-heading .pricing-sprite")
      .addClass("close-icon");
  } else {
    $("." + b).addClass("hide-elm");
    $(this)
...
function hgtEleUpdate() {
  $(".table-left .cmp_row").each(function () {
    $(this)
      ...
      .each(function (a) {
        $(this)
          .find(".cmp_cell:not(.grey-heading)")
          .each(function (b) {
            var e = $(this).outerHeight(true),
              c = $(this).index();
            $(".cur-hgtele").removeClass("cur-hgtele");
            $(".table-left .cmp_row,.table-right .cmp_row")
              .find(">ul:eq(" + a + ") .cmp_cell:eq(" + c + ")")
              .addClass("cur-hgtele");
            var d = 0;
...
if (currentUrlLang != "") {
  $(".business_sec .business_icons")
    .attr("src", "/sites/zweb/images/crm/zpricing-brands-bg-ot.svg")
    .attr("data-src", "/sites/zweb/images/crm/zpricing-brands-bg-ot.svg");
}
