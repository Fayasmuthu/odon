if (
    ...
    global_getUrlParam("itpricingtest") == ""
  ) {
    const prodSkipForIT = ["crm"];
    if (!prodSkipForIT.includes(customvar.productName)) {
      let _itRedirectionURL =
        window.location.pathname.replace("/it/", "/") + "?zredirect=f";
  ...
  $(document).ready(function () {
    ...
    $(".product-block").each(function () {
      if ($(this).find(".feature-wrap").length > 0) {
        $(
          '<div class="zmore-features"><span class="zmore-features-show">' +
            Drupal.t("Show Features") +
            '</span><span class="zmore-features-hide">' +
            Drupal.t("Hide Features") +
            "</span></div>"
        ).insertAfter($(this).find(".main-box"));
      }
    });
    ...
    if ($(".pricing-tab").length > 0) {
      $(
        '<div class="zmobile-mon-yearly-container"><select id="zmobile-mon-yearly"><option value="M">' +
          Drupal.t("Monthly") +
          '</option><option value="Y">' +
          Drupal.t("Yearly") +
          "</option></select></div>"
      ).appendTo(".pricing-tab");
    ...
    $(".most-popular-plan")
      .parent(".content-wrap")
      .addClass("has-most-popular-plan");
    $(
      '<div class="zmost-popular"><em>*</em>' +
        Drupal.t(" Most Popular") +
        " <em>*</em></div>"
    ).insertBefore($(".most-popular-plan .plan-name"));
      ...
      $.getJSON(jsonFileDesctination)
        .done(function (w) {
          ...
          if (f.length > 0) {
            $.each(f, function (A) {
              $.getJSON(
                "https://" + getDomain + "/sites/" + i + "/json/pricing/" + f[A]
              ).done(function (B) {
                n(B);
              });
            });
      ...
      function n(w) {
        $.each(w.priceLists.priceSets, function (x) {
          if ($(".zpricegroup#" + w.priceLists.priceSets[x].setId).length > 0) {
            if (
              $(".zpricegroup#" + w.priceLists.priceSets[x].setId).attr(
                "data-price"
              ) == ""
            ) {
              $(".zpricegroup#" + w.priceLists.priceSets[x].setId).attr(
                "data-price",
                w.priceLists.priceSets[x].priceValue
              );
            }
          }
          if (
            $("[data-jsonid='" + w.priceLists.priceSets[x].setId + "']").length >
            0
          ) {
            if (
              $("[data-jsonid='" + w.priceLists.priceSets[x].setId + "']").attr(
                "data-price"
              ) == ""
            ) {
              $("[data-jsonid='" + w.priceLists.priceSets[x].setId + "']").attr(
    ...
    function t() {
      if (window.innerWidth >= 768) {
        ...
        $(".main-box").css({ "min-height": "inherit" });
      }
    }
    function c() {
      if (window.innerWidth >= 768) {
        ...
        $(".highlighted-box").css({ "min-height": "inherit" });
      }
    }
    function h(w) {
    ...
    function o() {
      $(".trynow-plan").each(function () {
        $(this).attr(
          ...
          $(this)
            .attr("href")
            .replace("zoho.com", Zdomain + "." + dcdomainOne)
        );
      });
      $.each(allProductlowerCase, function (w, x) {
        ...
        $.each(eachAppFromAllApps, function (y, z) {
          if (z != "" && z != null) {
            $(".zw-template-inner [href*='" + z + ".zoho.com']")
              .not(".trynow-plan")
              .each(function () {
                if (
                  $(this)
                    .attr("href")
                    .indexOf(z + ".zoho.com") >= 0 &&
                  $(this).attr("href").indexOf(".com.au") < 0
                ) {
                  OtherPrdPricingURL = $(this).attr("href");
                  OtherPrdPricingURLDc = OtherPrdPricingURL.replace(
                    "zoho.com",
                    Zdomain + "." + dcdomainOne
                  );
    ...
    $(window).on("load", function () {
      $(".zpricingfaq-pt-br").html(
        'Aceitamos pagamentos com Visa, American Express e PayPal. Para faturamento anual, aceitamos transferÃªncias internacionais. Para mais detalhes, envie um e-mail para <a href="mailto:vendas@zohocorp.com">vendas@zohocorp.com</a>.'
      );
      b();
      o();
      r();
    ...
    function r() {
      if (domainOne == "cn" || domainOne == "com.cn") {
        ...
        if (countryCodeToBeUsed == "-") {
          ...
          if (countryEu.indexOf(countryCodeToBeUsed) > -1) {
            if (
              countryCodeToBeUsed == "GB" &&
              currencyArray.indexOf("GBP") >= 0
            ) {
              $(".EUR, .GBP").addClass("showThis");
              q("GBP", "Â£");
            } else {
              if (
                countryCodeToBeUsed == "TR" &&
                currencyArray.indexOf("USD") >= 0
              ) {
                q("USD", "â‚¬");
              } else {
                      ...
                      if (countryCodeToBeUsed == "SG") {
                        ...
                        if (countryCodeToBeUsed == "AE") {
                          ...
                          if (countryCodeToBeUsed == "SA") {
                            if (langsrc == "/pt-br/") {
                              if (
                                currencyArray.indexOf("OLDUSD") >= 0 &&
                                l.indexOf(productName) > -1
                              ) {
                                q("OLDUSD", "US$");
                              } else {
                                q("USD", "US$");
      ...
      if (
        $(".changeCurrency.showThis").length <= 1 &&
        $(".pricing-tab-wrap").length == 0
      ) {
        $(".changePrice").hide();
      }
      b();
    ...
    function q(z, x) {
      ...
      $(".zpricegroup").each(function () {
        ...
        if ($.isNumeric(E[w])) {
          var C = E[w];
          $(".product-block .price .zcurrency-symbol").css(
            "text-transform",
            "uppercase"
          );
          if (z == "MXN" || z == "IDR") {
          ...
          if (z == "AUD" && countryCodeToBeUsed == "AU") {
            if ($.trim($(".ztax-container > p#z-notax").text()) == "") {
              $(".ztax-container > p#z-notax")
                .text(Drupal.t("Prices mentioned are inclusive of local taxes."))
                .show();
            } else {
          ...
          if (z == "EUR" || z == "IDR") {
            ...
            if (C.indexOf(",") != -1) {
              ...
              if (z == "IDR") {
                if (C.length > 3) {
                  lastFourCharacters =
                    "<em class='zidr_small'>" + C.substr(-4) + "</em>";
    ...
    if (
      $("body").hasClass("lang-zh-hans") &&
      $(".pricing-tab .yearly").length > 0
    ) {
      $(".pricing-tab .yearly").trigger("click");
    }
    $(".feature-box ul li.has-tooltip, .highlighted-box p.has-tooltip").each(
      function (w, x) {
        $(this).hover(function () {
          $(this)
            .find("span.price-tooltip")
            .not(".zpricegroup, .ztogglegroup")
            .addClass("active");
        });
  ...
  $(".pricing-faq ul li h3").on("click", function () {
    if ($(window).width() <= 767) {
      if ($(this).parent().hasClass("zactive")) {
        ...
        $(
          ".pricing-faq ul li.zactive > p, .pricing-faq ul li.zactive > .faq-list"
        ).slideUp();
