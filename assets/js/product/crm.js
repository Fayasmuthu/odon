...
function z_set_account_src(g) {
    ...
    if (dcdomainOne == "in" || (countryEu.indexOf(CountryCode) > -1) || dcdomainOne == "ca") {
        k = "&mode=24";
    ...
    if (getParameterByName("local_instance") != "" && getParameterByName("local_instance") != null && ZUrlDomain != "zoho") {
        serviceURL = "&serviceurl=https%3A%2F%2F" + crm_instance + "." + Zdomain + "." + dcdomainOne + "%2Fcrm%2FShowHomePage.do%3FTODO%3DaddUser" + langChange + "%26fromPage=CRM_Signup%26isNewSignup=true" + vertical_param
    } else {
        serviceURL = "&serviceurl=https%3A%2F%2Fcrm." + Zdomain + "." + dcdomainOne + "%2Fcrm%2FShowHomePage.do%3FTODO%3DaddUser" + langChange + "%26fromPage=CRM_Signup%26isNewSignup=true" + vertical_param
    }
    var e = getParameterByName("src");
    if (e == "zohortcrmtraining" || presentUrl.indexOf("/crm/lp/worlds-favorite-crm.html") > -1 && presentUrl.indexOf("source_from=dream-force")) {
    ...
    var h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=ent" + j;
    if (g == "standard") {
        h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=std" + j
    } else {
        if (g == "enterprise") {
            h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=ent" + j
        } else {
            if (g == "msignup") {
                var l = $.iamUrl();
                h = customvar.alink + "servicename=" + serviceNAME + c + k + "&use_homeurl=true&serviceurl=" + l
            } else {
                if (g == "ultimate") {
                    h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=ultimate" + j
                } else {
                    if (g == "professional") {
                        h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=pro" + j
                    } else {
                        if (g == "free") {
                            h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + "%26plan=free" + j
                        }
                    }
                }
            }
    ...
    if ((presentUrl.indexOf("/crm/cx-platform/") > -1) || presentUrl.indexOf("signup-new.html") > -1 || presentUrl.indexOf("srcPlan=NewCRM") > -1 || $("body").find(".ncrm-btn").length > 0 || $(".czone-dc").hasClass("ncrm-signup")) {
        serviceNAME = "CRMPlus";
        serviceURL = "&serviceurl=https%3A%2F%2Fzcrm." + Zdomain + "." + dcdomainOne + "%2Findex.do%3F" + langChange;
        customvar.alink = "https://accounts." + Zdomain + "." + dcdomainOne + "/register/script?temp_redirect=true&load_country=false&";
        if (ZUrlDomain == _lz) {
            serviceURL = "&serviceurl=https%3A%2F%2Fnewcrmplus." + Zdomain + "." + dcdomainOne + "%2Findex.do" + langChange;
            customvar.alink = "https://accounts." + Zdomain + "." + dcdomainOne + "/register/script?temp_redirect=true&load_country=false&"
        }
        var f = "";
        if (getParameterByName("edition") != "" || getParameterByName("edition") != null) {
            f = "%26edition=" + getParameterByName("edition")
        }
        addParam = "%26solution=" + getParameterByName("solution") + f;
        if (getParameterByName("solution") == "" || getParameterByName("solution") == null) {
            addParam = ""
        }
        h = customvar.alink + "servicename=" + serviceNAME + c + k + serviceURL + addParam
    }
    ...
    $("#signupform").zaSignUp({
        validator: {
            ...
            messages: {
                ...
                x_company: {
                    required: companyrequire
                },
                x_empCount: {
                    required: usersrequire,
                    isValidUser: usersrequire
                },
                x_API_Users: {
                    required: usersrequire,
                    isValidUser: usersrequire
        ...
        oncomplete: function(d) {
            ...
            if (d == $.fn.zaSignUp.SIGNUP_STATE.ERROR || d == $.fn.zaSignUp.SIGNUP_STATE.BACKTO_SIGNUP) {
                a.val(getStartedTranslt);
                a.removeClass("disabled-z-signup");
                $(".loader_popupSec").removeClass("active");
                $(".socl-signup, .zcrm_split, .sgn-hdr, .signup-box h1").removeClass("zhide-common")
            } else {
                if (d == $.fn.zaSignUp.SIGNUP_STATE.OTP_INITIATED) {
                    ...
                    $(".socl-signup, .zcrm_split, .sgn-hdr, .signup-box h1").addClass("zhide-common")
                } else {
